...
import {
  ...
  Pencil, Archive, GraduationCap, X, AlertTriangle, RotateCcw, FileEdit,
} from 'lucide-react'
...
export default function CampusHiringPage() {
  ...
  // Read localStorage draft for Campus hiring
  const localDraft = useMemo(() => {
    try {
      const raw = localStorage.getItem('campaign_draft_CAMPUS')
      if (!raw) return null
  ...
  const discardLocalDraft = () => {
    localStorage.removeItem('campaign_draft_CAMPUS')
  ...
  const dbCampaigns = allCampaigns.filter((c: any) => c.hiringType === 'CAMPUS')
  ...
  // Auto-close expired campaigns once per data load (use a ref to avoid mutation loop)
  ...
  return (
    <div className="fade-in">
      ...
      {hasDraft && (
        <div style={{
          background: 'linear-gradient(135deg, rgba(99,120,255,0.12) 0%, rgba(139,92,246,0.08) 100%)',
          ...
          <FileEdit size={18} style={{ color: 'var(--primary)', flexShrink: 0 }} />
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: '0.875rem', color: 'var(--cream)' }}>
              You have an unsaved Campus campaign draft
            </div>
